...
const REUSE_TIPS = {
  plastik_pet:      ["Potong menjadi pot tanaman mini untuk balkon", "Jadikan tempat pensil atau alat tulis", "Lubangi bagian bawah, isi tanah, tanam benih microgreens"],
  plastik_hdpe:     ["Gunakan ulang sebagai wadah sabun refill", "Potong menjadi sekop mini untuk berkebun", "Jadikan ember kecil untuk menyimpan perkakas"],
  plastik_campuran: ["Kumpulkan dan padatkan menjadi eco brick", "Jadikan bahan isian bantal dekorasi", "Gunakan sebagai material pengisi pot besar"],
  kertas_bersih:    ["Lipat menjadi origami atau hiasan dinding", "Gunakan sebagai pembungkus kado unik", "Buat sketchbook sederhana dari kertas bekas"],
  kertas_kotor:     ["Sobek kecil-kecil untuk bahan kompos", "Gunakan sebagai lapisan dasar pot tanaman", "Jadikan bahan papier-mâché untuk kerajinan anak"],
  organik:          ["Olah menjadi pupuk kompos rumahan", "Jadikan pakan ternak atau ikan", "Gunakan kulit buah sebagai pewangi alami"],
  kaca_utuh:        ["Jadikan vas bunga atau tempat lilin", "Gunakan sebagai wadah penyimpanan bumbu dapur", "Cat dan jadikan dekorasi rumah unik"],
  kaca_pecah:       ["Bungkus rapat dengan koran sebelum dibuang", "Pecahan halus bisa dijadikan mosaik dekorasi", "Serahkan ke pengepul kaca khusus"],
};
...
const ReusePage = () => {
  ...
  const handleLog = async () => {
    try {
      ...
      await api.post("/actions/", {
        action_type:   "reuse",
        route:         "mandiri",       // ✅ WAJIB — backend baru
  ...
  if (success) return <PendingBanner onHome={() => navigate("/dashboard")} onHistory={() => navigate("/history")} />;
    ...
    <section className="min-h-screen bg-slate-50 py-12 px-6">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500">Ide kreatif untuk mengolah <span className="font-semibold text-slate-700">{wasteLabel}</span></p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-6 space-y-4">
          ...
          <p className="text-sm text-slate-500">Catat aksimu — poin diberikan setelah diverifikasi admin.</p>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Ceritakan apa yang kamu buat... (opsional)" rows={3} maxLength={300} className="w-full border rounded-xl p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-amber-400" />
          {error && <p className="text-sm text-red-500">{error}</p>}
          <button onClick={handleLog} disabled={loading} className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white font-semibold rounded-xl transition-colors disabled:opacity-50">
            {loading ? "Menyimpan..." : "✓ Catat Aksi"}
          </button>
        </div>
      </div>
    </section>
