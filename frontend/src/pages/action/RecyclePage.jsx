...
import { Recycle, Clock, ArrowLeft } from "lucide-react";
import api from "@/lib/axios";

const RECYCLE_INFO = {
  plastik_pet: {
    desc: "Plastik PET (kode #1) adalah salah satu plastik paling mudah didaur ulang. Biasanya berwarna bening atau biru muda.",
    steps: ["Bersihkan dari sisa makanan atau minuman", "Lepas tutup botol (beda jenis plastik)", "Gepengkan untuk menghemat tempat", "Kumpulkan lalu serahkan ke bank sampah atau pengepul"],
  },
  plastik_hdpe: {
    desc: "Plastik HDPE (kode #2) biasa ditemukan pada botol sabun, sampo, dan jerigen. Mudah didaur ulang dan bernilai jual.",
    steps: ["Bilas hingga bersih dari sisa produk", "Biarkan kering sebelum dikumpulkan", "Serahkan ke bank sampah atau pengepul plastik"],
  },
  kertas_bersih: {
    ...
    steps: ["Pastikan kertas kering dan bersih", "Pisahkan dari plastik atau staples", "Bundel atau masukkan ke dalam kardus", "Serahkan ke bank sampah atau pengepul kertas"],
  },
  kaca_utuh: {
    ...
    steps: ["Bersihkan dari sisa makanan atau minuman", "Jangan campur dengan kaca pecah", "Serahkan ke pengepul kaca atau bank sampah"],
  },
};
...
const DEFAULT_INFO = {
  ...
  steps: ["Bersihkan dari kontaminan", "Kumpulkan dalam jumlah yang cukup", "Serahkan ke bank sampah atau mitra daur ulang"],
};
    ...
    <section className="min-h-screen bg-slate-50 py-12 px-6">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-6 space-y-4">
          ...
          <p className="text-sm text-slate-500">Catat ke mana kamu menyerahkan sampah — poin diberikan setelah diverifikasi admin.</p>

          <div className="space-y-1">
            <label className="text-xs font-medium text-slate-600">Nama bank sampah / fasilitas (opsional)</label>
            <input type="text" value={partnerName} onChange={(e) => setPartnerName(e.target.value)} placeholder="cth: Bank Sampah Induk Yogyakarta" className="w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
          </div>

          <div className="space-y-1">
            <label className="text-xs font-medium text-slate-600">Catatan (opsional)</label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Tambahkan catatan..." rows={2} maxLength={300} className="w-full border rounded-xl p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-emerald-400" />
          </div>

          {error && <p className="text-sm text-red-500">{error}</p>}

          <button onClick={handleLog} disabled={loading} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-50">
